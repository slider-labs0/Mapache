...
## [ok] Phase 5 - Messaging Integrations
...
## [ok] Phase 6 - Advanced Security Tools
...
## [ok] Phase 7 - Multi-Model Routing (LIVE)
...
## [ok] Phase 8 - Verifier / Reflection (LIVE, opt-in)
...
## [ok] Post-Phase-8 - Differentiators, Decepticon parity & the agent-loop upgrade
...
- **Rules-of-Engagement guardrails (feature J)** - an optional `scope.json`
  ...
  CLI shows a startup banner, a `/scope` command, and a live `[blocked] RoE: refused …`
...
|---|---|---|
| 1 | Machine IP changed mid-session | [ok] Fixed - new IP overrides target + clears stale state |
| 2 | `nmap_scan` called without `target=` | [ok] Handled - arg backfill + prompt enforcement |
| 3 | Ollama XML error, 33 schemas overflow | [ok] Handled - phase-based tool subsetting |
| 4 | Metasploit attempted before scan | [ok] Mitigated - phase gating + workflow prompt |
| 5 | No open ports found | [ok] Was a symptom of #1 (scanning the dead IP) |

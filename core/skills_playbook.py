...
OSINT_SKILL = Skill(
    ...
    body=(
        ...
        "(`site:`, `filetype:`) for exposed docs. For exposed-host discovery use "
        "`crtsh_search(query=\"%.<domain>\")` (keyless Certificate-Transparency subdomain "
        "enumeration, always available) and - when configured - ZoomEye via its MCP "
        "(tools `mcp__zoomeye__*`) for device/service search by product/port/country "
        "(enable in mcp.json with a free ZOOMEYE_API_KEY). Prefer these; do not abandon "
        "the task if one source needs a key you don't have - pivot to the keyless one.\n"

...
OSINT_SKILL = Skill(
    ...
    body=(
        ...
        "(`site:`, `filetype:`) for exposed docs. NOTE on Shodan: the keyed API "
        "(shodan_host/shodan_search) needs SHODAN_API_KEY - if it is missing the tool "
        "says so plainly; that is a 'set the key' step, NOT an unbeatable Cloudflare "
        "wall, so do not abandon the task over it. For a per-IP lookup with NO key, use "
        "`shodan_internetdb(ip=...)` (free InternetDB: ports/hostnames/CVEs). Resolve a "
        "domain to an IP first (e.g. web_fetch/shell), then feed the IP to it. "
        "FREE Shodan alternatives: `crtsh_search(query=\"%.<domain>\")` (keyless "
        "Certificate-Transparency subdomain enumeration, always available) and - when "
        "configured - ZoomEye via its MCP (tools `mcp__zoomeye__*`) for free-tier "
        "device/service search, the closest Shodan-search substitute (enable in mcp.json "
        "with a free ZOOMEYE_API_KEY). Prefer these over giving up when Shodan is "

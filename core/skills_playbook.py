...
DARKWEB_SKILL = Skill(
    ...
    body=(
        ...
        "OFFLINE - or tor_fetch says the address is correct but the site is offline / "
        "behind DDoS protection (e.g. 'illegal request line' from Dread's EndGame) - then "
        "the service is simply DOWN. REPORT it as currently unreachable and STOP. Do NOT "
        "web_search the clearnet for 'alternative dread links' (that leaks intent and "
        "returns scams/clones) and do NOT loop trying more directories. Use `tor_fetch`/"
        "browser to open a specific live .onion, not to search.\n"
